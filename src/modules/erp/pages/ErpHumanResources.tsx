...
import {
  UserPlus, UserCog, Shield, FileText, Download, Banknote, Calculator,
  Info, Users, Wallet, TrendingUp, CreditCard, AlertTriangle, CheckCircle2,
  Pencil, BarChart3, PrinterIcon, Eye, ShoppingCart, Package, Truck,
} from "lucide-react";
...
interface Employee {
  ...
  birthDate: string; nationality: string; maritalStatus: string; childrenCount: number;
  address: string; phone: string; email: string;
  niu: string; cnssNumber: string; idCardType: string; idCardNumber: string;
...
interface PayrollCalc {
  ...
  netSalary: number;
  cnssEmployer: number; totalLaborCost: number;
}
...
const EMPTY_EMP: Partial<Employee> = {
  ...
  address: "", phone: "", email: "",
  niu: "", cnssNumber: "", idCardType: "CNI", idCardNumber: "",
  rib: "", bankName: "", bankAccount: "",
  department: "Commerce", position: "", contractType: "CDI",
...
const ERP_ROLES = [
  { value: "erp_admin", label: "Administrateur ERP" },
  { value: "erp_manager", label: "Responsable Boutique" },
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function ErpHumanResources() {
  ...
  const [userForm, setUserForm] = useState({ name: "", email: "", password: "", role: "caissier" });
  ...
  const kpis = useMemo(() => {
    ...
    const totalTOL = monthPays.reduce((s, p) => s + (p.totalLaborCost || 0), 0);
  ...
  const handleSaveEmployee = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: editingEmp ? "Employé modifié" : "Employé ajouté" });
    } catch (err: any) {
      toast({ variant: "destructive", title: "Erreur", description: err.message });
    }
  };
  ...
  const statusBadge = (status: string) => {
    if (status === "active") return <Badge className="bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">Actif</Badge>;
  ...
  return (
    <div className="space-y-6 p-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2">
            ...
            Ressources Humaines — Commerce & ERP
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Gestion du personnel commercial : caissiers, livreurs, gestionnaires de stock
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Effectif actif", value: kpis.active, icon: Users, color: "text-emerald-600", bg: "bg-emerald-500/10" },
          { label: "Masse salariale brute", value: fmt(kpis.totalBase) + " CFA", icon: Banknote, color: "text-teal-600", bg: "bg-teal-500/10" },
          { label: "Net versé ce mois", value: fmt(kpis.totalNet) + " CFA", icon: Wallet, color: "text-blue-600", bg: "bg-blue-500/10" },
          { label: "Coût total employeur", value: fmt(kpis.totalTOL) + " CFA", icon: TrendingUp, color: "text-violet-600", bg: "bg-violet-500/10" },
        ].map((kpi, i) => (
      ...
      <Tabs defaultValue="employees" className="w-full">
        <TabsList className="w-full max-w-lg grid grid-cols-3 bg-muted/50">
          ...
          <TabsTrigger value="access">Accès IT</TabsTrigger>
        </TabsList>

        {/* ── Personnel ── */}
        <TabsContent value="employees" className="mt-4 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-base font-bold">Personnel Commerce ({employees.length})</h2>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => exportToCSV(employees, "Personnel_ERP_KIAM")}>
                <Download className="h-4 w-4 mr-1" /> CSV
              </Button>
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={openAddEmp}>
                <UserPlus className="h-4 w-4 mr-1" /> Recruter
              </Button>
            </div>
          </div>
                  ...
                  {isLoading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                        ...
                        <p className="text-xs mt-1">Cliquez sur "Recruter" pour créer un dossier</p>
                      </TableCell>
                    </TableRow>
                  ) : employees.map(emp => (
                    <TableRow key={emp.id} className="group">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-600 font-black text-xs">
                            {emp.name[0] || "?"}
                          </div>
                          <div>
        ...
        <TabsContent value="payroll" className="mt-4 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => exportToCSV(payrolls, "Paie_ERP_KIAM")}>
                <Download className="h-4 w-4 mr-1" /> CSV
              </Button>
              <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => { setPayrollSim(null); setPayrollDialogOpen(true); }}>
                  ...
                  {payrolls.length === 0 ? (
                    ...
                    return (
                      <TableRow key={pay.id}>
                        ...
                        <TableCell className="font-mono text-sm text-violet-600">{fmt(pay.totalLaborCost)}</TableCell>
                        <TableCell>
                          <Badge className={pay.status === "paid" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20 border" : "bg-amber-500/10 text-amber-600 border-amber-500/20 border"}>
                            {pay.status === "paid" ? "Payé" : "Brouillon"}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" className="gap-1 text-blue-600" onClick={() => { setSelectedPayroll(pay); setSelectedEmployee(employees.find(e => e.id === pay.employeeId) || null); setPayslipOpen(true); }}>
        ...
        <TabsContent value="access" className="mt-4">
          <Card className="border-border/50">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div>
                <CardTitle className="text-base flex items-center gap-2"><Shield className="h-4 w-4 text-rose-500" /> Accès au Logiciel ERP</CardTitle>
                <CardDescription>Comptes autorisés à se connecter au système Kiam ERP</CardDescription>
              </div>
              <Button size="sm" className="bg-rose-600 hover:bg-rose-700 text-white gap-2" onClick={() => setUserDialogOpen(true)}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {editingEmp ? "Modifier le dossier" : "Nouveau dossier employé — Commerce"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSaveEmployee} className="space-y-4 pt-2">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5"><Label className="text-xs font-bold">Département</Label>
                <Select value={empForm.department || "Commerce"} onValueChange={v => setEmpForm({ ...empForm, department: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{DEPARTMENTS_ERP.map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5"><Label className="text-xs font-bold">Poste *</Label>
                <Select value={empForm.position || ""} onValueChange={v => setEmpForm({ ...empForm, position: v })}>
                  <SelectTrigger><SelectValue placeholder="Sélectionner..." /></SelectTrigger>
                  <SelectContent>{POSITIONS_ERP.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
                </Select>
              </div>
      ...
      <Dialog open={payrollDialogOpen} onOpenChange={setPayrollDialogOpen}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Calculator className="h-5 w-5 text-blue-500" /> Générer un bulletin de paie
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleGeneratePayroll} className="space-y-4 pt-2">
            <div className="space-y-1.5"><Label className="text-xs font-bold">Employé *</Label>
            ...
            {payrollSim && (
              <div className="space-y-2 bg-muted/50 rounded-xl p-4">
                ...
                <div className="flex justify-between bg-blue-50 dark:bg-blue-950/30 rounded-lg px-3 py-2"><span className="text-xs">Salaire Brut</span><span className="font-bold">{fmt(payrollSim.grossSalary)} CFA</span></div>
            ...
            <div className="flex justify-between pt-2 border-t">
              ...
              <Button type="submit" disabled={!payrollSim || !payrollForm.employeeId} className="bg-blue-600 hover:bg-blue-700 text-white gap-2">
          ...
          {selectedPayroll && selectedEmployee && (
            <div className="space-y-4 pt-2">
              <div className="flex justify-between items-start border-b-2 border-slate-900 pb-3">
                <div>
                  <h2 className="text-xl font-black uppercase">BULLETIN DE PAIE — ERP</h2>
              ...
              <Table className="border border-slate-200 text-sm">
                <TableHeader className="bg-slate-100">
                  <TableRow>
                    ...
                    <TableHead className="text-right font-black text-slate-900">GAINS (CFA)</TableHead>
                    <TableHead className="text-right font-black text-slate-900">RETENUES (CFA)</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  ...
                  {selectedPayroll.bonusesTotal > 0 && <TableRow><TableCell className="text-slate-600">Primes & Commissions</TableCell><TableCell className="text-right font-mono">{fmt(selectedPayroll.bonusesTotal)}</TableCell><TableCell /></TableRow>}
                  ...
                  <TableRow><TableCell className="text-rose-700">CNSS Salariale (2.8%)</TableCell><TableCell /><TableCell className="text-right font-mono text-rose-700">-{fmt(selectedPayroll.cnssEmployee)}</TableCell></TableRow>
              ...
              <p className="text-center text-xs text-slate-400 border-t pt-3">Kiam SaaS — Module RH Commerce</p>
            </div>
          )}
          <div className="flex justify-end gap-2 mt-2">
            <Button variant="outline" onClick={() => setPayslipOpen(false)}>Fermer</Button>
            <Button className="bg-blue-600 hover:bg-blue-700 text-white gap-2" onClick={() => window.print()}>
      ...
      <Dialog open={userDialogOpen} onOpenChange={setUserDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Shield className="h-5 w-5 text-rose-500" /> Nouvel accès utilisateur ERP</DialogTitle></DialogHeader>
          <form onSubmit={handleAddUser} className="space-y-4 pt-2">
            ...
            <div className="space-y-1.5"><Label className="text-xs font-bold">Rôle</Label>
              <Select value={userForm.role} onValueChange={v => setUserForm({ ...userForm, role: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{ERP_ROLES.map(r => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="flex justify-end gap-2 pt-2 border-t">
